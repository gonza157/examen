...
void harcodeoMusico(eMusico* client,int len)
{
    ...
    client[3].edad=22;
    client[3].idinstrumento=2;
    client[3].idorquesta=1;
...
int printMusicoConinstrumento(eMusico* list,int posicion,eInstrumento* inst,int leninstrumentos)
{
    int nInstrumento;
    int i=0;
...
int listarMusicosConinstrumento(eOrquesta* orquesta,int lenorquesta,eMusico* musico,int lenmusico,eInstrumento* instrumento, int leninstrumento)
{
    ...
    for(i=0; i<lenmusico; i++)
    {
        if(idorquesta==musico[i].idorquesta && musico[i].isEmpty==0)
        {
            printMusicoConinstrumento(musico,i,instrumento,leninstrumento);
            retorno=0;
        }
    }
    if(retorno==-1)
